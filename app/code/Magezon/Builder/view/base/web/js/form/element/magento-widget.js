define([
	...
	return {
		controller: function($scope, magezonBuilderService, $sce) {
			...
			$.ajax({
				url: magezonBuilderService.getUrl('mgzbuilder/widget/index'),
				type:'POST',
				data: {
			...
			$scope.$watch('html', function(value) {
				if ($scope.model[$scope.options.key] && value) {
	                ...
	                if (widgetValue) {
						...
						$.ajax({
							url: magezonBuilderService.getUrl('mgzbuilder/widget/loadOptions'),
							type:'POST',
							data: {
			...
			$scope.updateValue = function() {
				...
				if (form.valid() && $scope.ajax) {
					...
					$scope.disableActionButtons();
					$.ajax({
						url: magezonBuilderService.getUrl('mgzbuilder/widget/buildWidget'),
						type:'POST',
						data: form.serialize(),
						success: function(result) {
							$scope.ajax = true;
			...
			$(document).on('click', '.rule-param-apply,.rule-param-remove,.data-grid ._clickable,#select_widget_type', function() {
				$scope.updateValue();
			});
